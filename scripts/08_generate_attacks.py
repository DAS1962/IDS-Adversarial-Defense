...
"""
Génération des 6 attaques adversariales sur le baseline v4.
...
Attaques implémentées (référence : Awad et al. 2025) :
  ...
  - JSMA (Papernot 2015)       : feature-based, L0, theta=0.1, gamma=1.0
  - C&W (Carlini & Wagner 2016): optimization-based, L2, 9 iter
...
# torchattacks pour FGSM, BIM, PGD
import torchattacks

# ART pour DeepFool, JSMA, C&W
from art.attacks.evasion import DeepFool, SaliencyMapMethod, CarliniL2Method
from art.estimators.classification import PyTorchClassifier

# Ajouter src/ au path pour importer le modèle
...
# Hyperparamètres des attaques (fidèles au papier)
ATTACK_CONFIGS = {
    "FGSM": {"eps": 0.2},
...
def load_baseline_model(device):
    """
    Charge le baseline v4 depuis le checkpoint.
    Le modèle est mis en mode eval() car on ne l'entraîne plus.
    """
...
def load_test_data():
    ...
    Le test est utilisé tel quel (distribution naturelle, sans SMOTE).
    """
...
def create_art_classifier(model, device):
    """
    Wrappe le modèle PyTorch pour ART (nécessaire pour DeepFool, JSMA, C&W).
    """
...
def generate_torchattacks_batch(attack, X_batch, y_batch, device):
    """Génère les adversariaux pour un batch avec torchattacks."""
...
def generate_torchattacks(attack, X, y, device, name):
    """
    Génère les adversariaux pour tout le dataset avec torchattacks (FGSM, BIM, PGD).
    Traite par batchs pour économiser la mémoire GPU.
    """
    ...
    for i in range(n_batches):
        ...
        X_adv[start:end] = generate_torchattacks_batch(attack, X_batch, y_batch, device)

        if (i + 1) % 100 == 0 or (i + 1) == n_batches:
            pct = 100 * (i + 1) / n_batches
            elapsed = time.time() - debut
            print(f"  Batch {i+1}/{n_batches} ({pct:.1f}%) - {elapsed:.1f}s écoulées")

    duree = time.time() - debut
    print(f"  {name} termine en {duree:.1f}s\n")
    return X_adv


def generate_art_attack(attack, X, y, name):
    """
    Génère les adversariaux pour tout le dataset avec ART (DeepFool, JSMA, C&W).
    """
    print(f"Génération {name}...")
    debut = time.time()
...
def evaluate_attack(model, X_adv, y_true, device, name):
    """
    Évalue le baseline sur les exemples adversariaux.
    Calcule accuracy, precision, recall et F1 (macro et weighted).
    Le classification_report affiche les métriques par classe.
    """
    ...
    # Precision, Recall, F1 en versions macro et weighted
    ...
    # Rapport par classe (utile pour l'analyse détaillée)
...
def save_adversarial_examples(X_adv, name, output_dir):
    """Sauvegarde les exemples adversariaux d'une attaque en pickle."""
...
def print_summary_table(all_results, baseline_metrics):
    """Affiche un tableau récapitulatif de toutes les attaques."""
    print("\n" + "=" * 100)
    ...
    print(
        ...
        f"{baseline_metrics['precision_macro']:>12.4f} "
        f"{baseline_metrics['precision_weighted']:>12.4f} "
        f"{baseline_metrics['recall_macro']:>11.4f} "
        f"{baseline_metrics['recall_weighted']:>11.4f} "
        f"{baseline_metrics['f1_macro']:>10.4f} "
        f"{baseline_metrics['f1_weighted']:>10.4f}  (baseline)"
    )
    print("-" * 100)
    ...
    for r in all_results:
        print(
            ...
            f"{r['precision_weighted']:>12.4f} "
            f"{r['recall_macro']:>11.4f} "
            f"{r['recall_weighted']:>11.4f} "
            f"{r['f1_macro']:>10.4f} "
...
def evaluate_baseline_on_clean(model, X_test, y_test, device):
    """
    Évalue le baseline sur les données propres.
    Sert de référence pour comparer avec les attaques.
    """
...
def main():
    print("Génération des attaques adversariales sur le baseline v4")
    ...
    # Wrapper ART pour les attaques qui en ont besoin
    art_classifier = create_art_classifier(model, device)

    # Évaluation baseline sur données propres (référence)
