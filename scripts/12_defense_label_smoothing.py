...
Le modèle est entraîné avec une cross-entropy adoucie (label_smoothing=0.1)
au lieu de la cross-entropy classique. Cela rend le modèle moins confiant
dans ses prédictions et donc plus robuste aux perturbations adversariales.

Après l'entraînement, on évalue le modèle sur les données propres et sur
les 6 attaques adversariales.

Référence : Szegedy et al. 2016, Awad et al. 2025.
"""
...
# Hyperparamètre de Label Smoothing (papier Awad 2025)
LABEL_SMOOTHING = 0.1
...
def main():
    ...
    out_path = LOG_DIR / f"defense_ls_{timestamp}.pkl"
    joblib.dump({
        "results": all_results,
        "history": history,
        "hyperparameters": {
